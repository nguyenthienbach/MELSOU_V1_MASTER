...
When project owner locks design, enters review, or begins checkout, room changes read-only. Returning to Studio can reopen it. No Duo voice recording/upload/merge exists in V1. Test invite security, max-two enforcement, lock expiration, disconnect/reconnect, concurrent different slots, same-slot collision, auth scope and checkout read-only behavior.
